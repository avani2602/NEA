...
public class King extends Piece{
    ...
    public King(int color, int col, int row) {
        ...
        if(color == GamePanel.WHITE){
            image = getImage("/Chess/resources/piece/w-king");
        }
        else{
            image = getImage("/Chess/resources/piece/b-king");
        }
    }

    public boolean canMove(int targetCol, int targetRow){
        if(isWithinBoard(targetCol, targetRow)){
            //king can move if movement ratio of columns to rowa is 1:1 and if movemetn sum of raws & colums is 1 (can only move up or one down)
